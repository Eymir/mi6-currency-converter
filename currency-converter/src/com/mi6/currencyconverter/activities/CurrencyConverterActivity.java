	...
	private void addMainLayout() {
    	...
    	if (usedCurrencies == null || usedCurrencies.size() == 0) {
			...
			adapter = new CurrencyConvertorArrayAdapter(
					getApplicationContext(), R.layout.convertor_listitem, displayedList, this);
	...
	public void convertAction(View view) {
		if ((displayedList != null) && (displayedList.size() > 0)) {
			if (isEditTextSelected()){
				alertDialog.setMessage((String)this.getString(R.string.alertDialog_messages_select_currency_field));
		        alertDialog.show();
			} else {
				if (!hasMainFieldData()) {
